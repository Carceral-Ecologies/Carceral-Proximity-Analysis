                 ...
                 tabPanel("Interactive Map", #tab panels will appear in the upper navigation
                          div(class="outer",
                              ...
                              absolutePanel(id = "controls", 
                                            ...
                                            hr(),
                                            p("Filter to carceral facilities with at least"),
                                            splitLayout(
                                              ...
                                              ), cellWidths = c("25%", "75%"), cellArgs = list(style = "vertical-align: top")
                                            ),
                                            splitLayout(cellWidths = c("30%", "70%"),
                                                        p("within"),
                                                        sliderInput("proximity_val", "meters", min = 0, max = 5000, value = 1000, step = 1000)
                                            ),
...
server <- function(input, output, session) {
  ...
  #The app will observe when a user selects a new state [CURRENTLY NOT DOING: and do two things - 1) populate the tracts variable with the census tracts shapefile for the state,] and 2) list the names of the carceral facilities associated with that state in the carceral facility search dropdown
  observeEvent(input$state, {
    ...
    #codes_filtered <- codes %>% filter(STATE_CODE == input$state) #Filter codes df to selected state
  ...
  #The app will observe when a user selects a new status, type, or capacity and updates the search select options
